...
if(isset($_POST['gistID'])){
    ...
    $post = new Posts();
    $p = json_decode($post->getByGistId($gist));
    if(sizeof($p->posts) > 0){
        $counter = (sizeof($p->posts) > 20)? 20:sizeof($p->posts);
        print '<div class="gist-container" id="list-post-container" style="padding: 0px 0px;">';
        print '<ul class="media-list media-list-users list-group" id="list-group-container">';
        for ($i = 0; $i < $counter; $i++)
        {
            print '<li class="list-group-item notification-list-item wow slideInUp" id="list-group-'.$p->posts[$i]->id.'">';
            ...
            for($j= 0; $j < sizeof($u->users); $j++)
            {
                ...
                if($p->posts[$i]->quotes > 0)
                {
                    ...
                    for ($h = 0; $h < sizeof($pt->posts); $h++)
                    {
                        ...
                        for ($g=0; $g < sizeof($qu->users); $g++)
                        {
                            ...
                            print '<div class="media w-100">';
                            print '<a href="/gist/'.$gist.'?#list-group-'.$pt->posts[$h]->id.'" style="display: flex;">';
                            ...
                            $quotePostContent = htmlspecialchars_decode(stripslashes(convertHashTag(convertMentions(nl2br(link_it($pt->posts[$h]->content))))));
                            print '<p id="quotePostContent"  style="line-height: 22px;">' . $quotePostContent . '</p>';
                            print '</div>';
                            print '</a>';
                            print '</div>';
                            print '</li>';
                ...
                print '</div>';
                print '</div>';
                print '<!--End mobile view-->';

                print '<div class="gist like-holder">';
                print '<p class="count-info" id="count-like">'.countFormat($p->posts[$i]->likes).'</p>';
                $l = new Likes();
                $jl = json_decode($l->getByUserAndPostId($p->posts[$i]->id, $_SESSION['id']));
                if(sizeof($jl->like) == 0)
                {
                    print '<button class="btn btn-outline-primary btn-sm float-right" data-post-user-id="'.$p->posts[$i]->user_id.'" data-post-id="'.$p->posts[$i]->id.'" data-sess-user-id="' . $_SESSION['id'] . '" data-user-id="' . $gist . '" data-gist-id="" id="like-btn" style="border-radius: 25px;"><i class="mdi mdi-thumb-up"></i> likes</button>';
                }
                else
                {
                    if ($p->posts[$i]->id == $jl->like[0]->post_id)
                        print '<button class="btn btn-outline-success btn-sm float-right" data-post-user-id="'.$p->posts[$i]->user_id.'" data-post-id="'.$p->posts[$i]->id.'"  data-sess-user-id="' . $_SESSION['id'] . '" data-user-id="' . $gist . '" data-gist-id="" id="liked-btn" style="border-radius: 25px;"><i class="mdi mdi-thumb-up"></i> liked</button>';
                    else
                        print '<button class="btn btn-outline-primary btn-sm float-right" data-post-user-id="'.$p->posts[$i]->user_id.'" data-post-id="'.$p->posts[$i]->id.'"  data-sess-user-id="' . $_SESSION['id'] . '" data-user-id="' . $gist . '" data-gist-id="" id="like-btn" style="border-radius: 25px;"><i class="mdi mdi-thumb-up"></i> likes</button>';
                }
                print '</div>';
                ...
                print '<li><a href="http://www.facebook.com/sharer.php?u=www.gistout.com/gist?gist='.$gist.'#list-group-'.$p->posts[$i]->id.'" target="_blank"><i class="mdi mdi-facebook brand-primary"></i> Facebook</a></li>';
                ...
                print '<div class="gist like-holder">';
                print '<p></p>';
                print '<button id="quote" class="btn btn-outline-success btn-sm float-right role" data-gist-id="'.$gist.'" data-purpose="'.$p->posts[$i]->id.'" data-purposeful="'.$p->posts[$i]->user_id.'" data-sess="'.$_SESSION['id'].'" data-toggle="modal" data-target=".bs-example-modal-lg" data-modal-role="quote" style="border-radius: 25px; margin-bottom: -3px;">';
                ...
                if($p->posts[$i]->user_id != $_SESSION['id'])
                {
                    ...
                    print 'Report';
                    print '</button>';
                    print '<ul class="dropdown-menu" role="menu" data-p-Id="'.$p->posts[$i]->id.'" data-post_user="'.$p->posts[$i]->user_id.'" data-sess="'.$_SESSION['id'].'">';
        ...
        if(sizeof($p->posts) > 20)
        {
            ...
            print '<div class="gist-container no-margin margin-top no-shadow" data-area="posts" id="gist-l-m" data-last-id="'.$last_id.'" data-gist="'.$gist.'">';
            print '<p>Load More</p>';
